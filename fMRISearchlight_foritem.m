function [varargout] = fMRISearchlight_foritem(fullBrainVols, justThisSession,justThisCondition,binaryMasks_nS, models, betaCorrespondence, userOptions)
%
...
if overwriteFlag
    ...
    %warp options are for normalization processing.
	warpFlags.interp = 1;
	warpFlags.wrap = [0 0 0];
	warpFlags.vox = userOptions.voxelSize2; % [3 3 3.75]
	warpFlags.bb = [-90 -126 -72; 91 91 109];%[-78 -112 -50; 78 76 85]; %this is the default one? Template bounding box is: [-90:91 -126:91 -72:109]
	...
	for subjectNumber = 1:nSubjects % and for each subject...
		...
		tic;% TIC and TOC functions work together to measure elapsed time. how many seconds for the analysis
		...
		if ischar(betaCorrespondence) && strcmpi(betaCorrespondence, 'SPM')
			betas = getDataFromSPM(userOptions); %only get the names of beta images. Please make sure that all subjects share the same model of first-level analysis.
		else
...
%		subjectMetadataStruct = spawnSPMStruct;
		...
		for maskNumber = 1:nMasks % For each mask...
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
			%let the specific stim in one condition

            for kk = 5*justThisCondition-4:5*justThisCondition
                dataprep = fullBrainVols(kk).(subject);
                fullBrainVolumes_tp = dataprep(:,1,:);
                fullBrainVols(kk).(subject) = fullBrainVolumes_tp;
            end
            kk = [5*justThisCondition-4:5*justThisCondition];
            for kk2 = 1:5               
                subdata(:,kk2,:) = fullBrainVols(kk(kk2)).(subject);
            end
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
			...
			singleSubjectVols = subdata;%fullBrainVols2.(subject); %so, still, do it one by one.
clear subdata
			% Do the searchlight! ZOMG, this takes a while...
            %%
			[rs, ns, searchlightRDMs.(subject)] = searchlightMapping_fMRI_foritem(singleSubjectVols,justThisSession, models,mask, userOptions, searchlightOptions); % ps are from linear correlation p-values, and so aren't too useful here.
			%%
			...
			for modelNumber = 1:numel(models)
                ...
                modelName = ['s',num2str(justThisSession),'c',num2str(justThisCondition)];
				%modelName = models(modelNumber).name; %avoid space in the modelname in the first space.
				...
				rMapMetadataStruct_nS.fname = fullfile(userOptions.rootPath, 'Maps', [userOptions.analysisName '_rMap_' maskName '_' modelName '_' subject '.nii']);
				...
				if isfield(userOptions, 'structuralsPath')
					...
					maskMetadataStruct_nS.fname = fullfile(userOptions.rootPath, 'Maps', [userOptions.analysisName '_nativeSpaceMask_' maskName '_' modelName '_' subject '.nii']);
					maskMetadataStruct_nS.descrip =  'Native space mask';
					maskMetadataStruct_nS.dim = size(mask); % mask.nii file
...
% 					wildFiles = replaceWildcards(fullfile(userOptions.structuralsPath, ['*' subject '*_seg_sn.mat']), '[[subjectName]]', subject);
                    wildFiles = replaceWildcards(fullfile(userOptions.structuralsPath, ['*_seg_sn.mat']), '[[subjectName]]', subject); % this probably need to change with SPM12 version
					...
					maskMetadataStruct_sS.fname = fullfile(userOptions.rootPath, 'Maps', [userOptions.analysisName '_commonSpaceMask_' maskName '_' modelName '_' subject '.nii']);
					...
					% Smooth it
					smoothingKernel_fwhm = userOptions.smoothingKernel_fwhm;%[10 10 10]; %10 mm FWHM. too large?
					...
					maskedDataMetadataStruct_nS.fname = fullfile(userOptions.rootPath, 'Maps', ['msw' userOptions.analysisName '_rMap_' maskName '_' modelName '_' subject '.nii']); % 'msw' for 'masked, smoothed, warped'
...
function [smm_rs, n, searchlightRDMs] = searchlightMapping_fMRI_foritem(fullBrainVolumes, justThisSession,models, mask, userOptions, localOptions)
	...
	localOptions = setIfUnset(localOptions, 'averageSessions', false); % so it is default to average across sessions.
	...
	%% Get parameters
	voxSize_mm = userOptions.voxelSize; %So, this should be the original voxel size?
	...
	% Prepare models
	modelRDMs_ltv = permute(unwrapRDMs(vectorizeRDMs(models)), [3 2 1]);%exclude meta information and transfer to lower-triangular vector (i.e., one dim) format
...
% 	smm_ps=nan([volSize_vox,nModelRDMs]);
	...
	for cMappingVoxI=1:nVox_mappingMask_request
		...
		if localOptions.averageSessions %here, run the first-oder RDMs
			...
			for session = 1:localOptions.nSessions
				...
				searchlightRDM = searchlightRDM + squareform(1-pdist(t_patsPerSession.(sessionId)(:,cIllValidVox_YspaceINDs),'correlation')); %refer to pdist function for other options.
			end%for:sessions
			searchlightRDM = searchlightRDM / localOptions.nSessions;%average RDM across sessions
		else
			searchlightRDM = squareform(1-pdist(t_pats(:,cIllValidVox_YspaceINDs), 'correlation'));%correlation, similarity
		end%if
		
		searchlightRDM = vectorizeRDM(searchlightRDM); %vectorize
        if localOptions.fisher
            for i = 1:numel(searchlightRDM)
                searchlightRDM(i) = fisherTransform(searchlightRDM(i));%z-transformed
            end
        end
		...
		try
            rs = sum(searchlightRDM);
			%[rs, ps] = corr(searchlightRDM', modelRDMs_ltv', 'type', 'Spearman', 'rows', 'pairwise'); %Here, run the second-order RDMs, using Spearman.
            %[rs, ps] = corr(searchlightRDM', modelRDMs_ltv', 'type', 'Pearson', 'rows', 'pairwise');
           % [rs, ps] = corr(searchlightRDM', modelRDMs_ltv', 'type', 'Kendall', 'rows', 'pairwise');
        catch
            rs = sum(searchlightRDM);
...
% 		if localOptions.fisher
% 			for i = 1:numel(rs)
% 				rs(i) = fisherTransform(rs(i));  %this is a key step, otherwise, one can not use the r values directly for statistical analysis. r maps are actually z mpas
% 			end%for:i
% 		end%if
